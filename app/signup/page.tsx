...
export default function SignUpPage() {
  const router = useRouter();
  const [formData, setFormData] = useState({
    fullName: '', email: '', grade: '', gender: '', password: '', confirmPassword: '',
  });
  const [showPw, setShowPw]   = useState(false);
  const [showCPw, setShowCPw] = useState(false);
  const [error, setError]     = useState('');
    ...
    <div className={`${outfit.variable} min-h-screen flex font-[var(--font-outfit)]`}>
      ...
      <div className="hidden lg:flex relative w-[48%] bg-[#3a5a09] overflow-hidden flex-col items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col items-center text-center px-12">
          ...
          <div style={{ animation: 'floatUp 4s ease-in-out infinite alternate' }}>
            <Image src="/login-illustration.svg" alt="Students" width={300} height={260}
              className="w-64 h-auto drop-shadow-2xl mt-[-65]" />
          </div>
          <h2 className="text-white text-3xl font-bold mt-6 leading-tight">
            Join SmartMathz<br />Today
          </h2>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-2 mb-7">
            {[1, 2].map((s) => (
              <div key={s} className="flex items-center gap-2 flex-1">
                ...
                {s < 2 && (
                  <div className={`flex-1 h-0.5 rounded-full ${step > s ? 'bg-[#7FB509]' : 'bg-gray-100'}`} />
                )}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              {/* Grade + Gender side by side */}
              <div className="grid grid-cols-2 gap-3">
                {/* <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Grade Level</label>
                  <select name="grade" value={formData.grade} onChange={handleChange}
                    className="w-full px-3 py-3 rounded-xl border border-gray-200 text-sm bg-white
                               focus:outline-none focus:ring-2 focus:ring-[#7FB509]/30 focus:border-[#7FB509]
                               text-gray-700 transition-all cursor-pointer">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              {formData.confirmPassword && (
                <p className={`text-xs font-semibold ${formData.password === formData.confirmPassword ? 'text-[#7FB509]' : 'text-red-500'}`}>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={handleSubmit} disabled={loading}
                  className="flex-grow py-3 rounded-xl bg-[#7FB509] hover:bg-[#6b970a] text-white font-bold text-sm
                             flex items-center justify-center gap-2 cursor-pointer
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            Already have an account?{' '}
            <Link href="/login" className="text-[#7FB509] font-semibold hover:underline">Sign in</Link>
          </p>

          <div className="mt-6 pt-5 border-t border-gray-100 text-center">
            <a href="https://smartmathz.com"
              className="text-xs text-gray-400 hover:text-[#7FB509] transition-colors">
              ← Go to SmartMathz Academy
            </a>
          </div>
        </div>
      </div>
