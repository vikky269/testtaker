...
// ── Price computation — accepts both adjuster AND custom session count ─────────
function computePriceWithAdjuster(
  standardHourlyRate: number,
  monthlyHours: number,        // base monthly hours from package
  adjuster: number,            // $ discount per hour
  customSessions?: number,     // optional override for sessions count
): ComputedPrice {
  const sessions           = customSessions ?? monthlyHours;
  const standardMonthlyFee = Math.round(standardHourlyRate * sessions);
  const smHourlyRate       = standardHourlyRate - adjuster;
  const smMonthlyFee       = Math.round(smHourlyRate * sessions);
