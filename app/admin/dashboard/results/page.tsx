...
// // Results & Recommendations — with live pricing preview and PDF output
...
// import { getPricingTable, computePrice, computeCustomPrice, type ComputedPrice, type GradePricing } from '@/app/utils/pricingData';
...
// // ── Pricing breakdown card (reused in modal) ──────────────────────────────────
// function PriceCard({ price, packageLabel }: { price: ComputedPrice; packageLabel: string }) {
//   return (
...
//             <td className="px-3 py-2 text-center text-sm font-semibold text-gray-700">{price.sessions}</td>
//             <td className="px-3 py-2 text-center text-sm font-bold text-yellow-600 bg-yellow-50">{price.sessions}</td>
//           </tr>
//           <tr className="border-t border-gray-100">
//             <td className="px-3 py-2 text-xs text-gray-500">Hourly Rate</td>
...
//           <p className="text-xs text-green-600">{price.savingsPercent}% Per Month</p>
//         </div>
//         <div className="text-right">
//           <p className="text-xs text-green-700 font-semibold">SmartMathz Investment</p>
//           <p className="text-xs text-green-600">${price.smInvestment} Per Month</p>
//         </div>
//       </div>
//     </div>
//   );
...
//   const [customMonthHrs, setCustomMonthHrs] = useState<number>(8);

//   // ── Data load ──────────────────────────────────────────────────────────────
//   useEffect(() => {
//     const load = async () => {
//       const { data, error } = await supabase.from('leaderboard').select('*').order('overall_score', { ascending: false });
...
//   const categoryCounts = LEARNING_CATEGORIES.map(c => ({ ...c, count: results.filter(r => getLearningCategory(r.overall_score).name === c.name).length }));
...
//       return computeCustomPrice(customStdRate, customMonthHrs);
//     }
...
//     return computePrice(pp);
//   }, [selectedPackage, pricingTable, customStdRate, customMonthHrs]);
...
//   const updateHours = (name: string, h: number) => setCustomSubjects(p => p.map(s => s.name === name ? { ...s, hours: h } : s));
...
//                         <div className="w-8 h-8 rounded-full bg-green-100 text-green-800 text-xs font-bold flex items-center justify-center flex-shrink-0">{r.full_name.charAt(0).toUpperCase()}</div>
//                         <div>
...
//                     <td className="px-4 py-3"><span className="text-xs bg-blue-50 text-blue-700 font-semibold px-2 py-0.5 rounded-full">{sugg.label}</span></td>
//                     <td className="px-4 py-3">
...
//                 </div>
//                 <button onClick={() => setSelectedStudent(null)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 cursor-pointer text-gray-400 text-lg">✕</button>
//               </div>
...
//                           {pkg.id === 'custom'
//                             ? 'Build your own'
//                             : pkg.hoursPerWeek !== null
//                               ? `${pkg.hoursPerWeek * 4}hrs/month`   // ← weekly × 4 = monthly
//                               : ''}
//                         </p>
//                       </button>
//                     ))}
...
//                 {/* ── Standard packages: show pricing table + price card ── */}
//                 {selectedPackage.id !== 'custom' && pricingTable && computedPrice && (
//                   <>
//                     {/* Rate comparison (all 3 packages shown for reference) */}
//                     <div>
//                       <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Pricing Reference — {cat.name} rates</p>
...
//                         ].map(({ label, pp }, i) => {
//                           const isActive = (selectedPackage.id === 'I' && i === 0) || (selectedPackage.id === 'II' && i === 1) || (selectedPackage.id === 'III' && i === 2);
//                           return (
...
//                               <p className="text-green-600">SM: <span className="font-semibold">${pp.standardHourlyRate - 20}/hr</span></p>
//                               <p className="text-gray-400 mt-1">${pp.standardMonthlyFee}/mo → <span className="text-green-600 font-semibold">${computePrice(pp).smMonthlyFee}/mo</span></p>
//                             </div>
//                           );
//                         })}
//                       </div>
...
//                     {/* Selected package full breakdown */}
//                     <PriceCard price={computedPrice} packageLabel={selectedPackage.label} />
//                   </>
//                 )}
...
//                           <input type="number" min={30} max={100} step={1} value={customStdRate}
...
//                           <p className="text-xs text-green-600 mt-1">SM rate: ${(customStdRate - 20).toFixed(2)}/hr</p>
//                         </div>
//                         <div>
//                           <label className="text-xs text-gray-500 mb-1 block">Monthly Hours</label>
//                           <select value={customMonthHrs} onChange={e => setCustomMonthHrs(parseInt(e.target.value))}
//                             className="w-full px-3 py-2 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-green-500/20 cursor-pointer">
//                             <option value={8}>8 hrs/month</option>
//                             <option value={16}>16 hrs/month</option>
//                             <option value={20}>20 hrs/month</option>
//                             <option value={12}>12 hrs/month</option>
//                             <option value={24}>24 hrs/month</option>
//                           </select>
//                         </div>
//                       </div>
...
//                     {/* Full breakdown card */}
//                     {computedPrice && <PriceCard price={computedPrice} packageLabel="Custom Package" />}
...
//                           <input type="number" min={1} max={5} step={1} value={newSubjectHours}
...
//                               <input type="number" min={1} max={5} step={1} value={s.hours}
...
//                               <span className="text-xs text-gray-400">hr/wk</span>
//                               <button onClick={() => removeCustomSubject(s.name)} className="text-red-400 hover:text-red-600 cursor-pointer">✕</button>
//                             </div>
//                           ))}
//                           <p className="text-xs text-gray-500">Total: <strong>{customSubjects.reduce((s, c) => s + c.hours, 0)}hrs/week</strong></p>
//                         </div>
//                       ) : (
...
import { getLearningCategory, LEARNING_CATEGORIES, type LearningCategory } from '@/app/utils/reviewUtils';
import { PACKAGES, CUSTOM_PACKAGE_SUBJECTS, getSuggestedPackage, generateRecommendationPDF, type PackageOption, type CustomPackageSubject } from '@/app/utils/generateRecommendation';
import { getPricingTable, computePrice, type ComputedPrice, type GradePricing, SM_ADJUSTER } from '@/app/utils/pricingData';
...
// Compute price with a custom adjuster value
function computePriceWithAdjuster(
  ...
  const smHourlyRate       = standardHourlyRate - adjuster;
  const smMonthlyFee       = Math.round(smHourlyRate * monthlyHours);
  const smBiweekly         = smMonthlyFee / 2;
  const savingsPercent     = Math.round(((standardMonthlyFee - smMonthlyFee) / standardMonthlyFee) * 100);
  const smInvestment       = standardMonthlyFee - smMonthlyFee;
  return { sessions: monthlyHours, standardHourlyRate, standardMonthlyFee, smHourlyRate, smMonthlyFee, smBiweekly, savingsPercent, smInvestment };
}

// ── PriceCard with adjustable adjuster ───────────────────────────────────────
function PriceCard({
  ...
  return (
    <div className="rounded-2xl border border-gray-100 overflow-hidden">
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-gray-50">
            ...
            <th className="px-3 py-2 text-center text-xs font-medium text-gray-500">Standard Rate</th>
            <th className="px-3 py-2 text-center text-xs font-bold text-green-700 bg-green-50">SmartMathz Offer</th>
            {/* Adjuster column header */}
            <th className="px-3 py-2 text-center text-xs font-medium text-red-400 w-28">Adjuster</th>
          </tr>
        </thead>
        <tbody>
          <tr className="border-t border-gray-100">
          ...
          <tr className="border-t border-gray-100">
            ...
            <td className="px-3 py-2 text-center text-sm font-semibold text-gray-700">${price.standardHourlyRate.toFixed(2)}</td>
            <td className="px-3 py-2 text-center text-sm font-bold text-yellow-600 bg-yellow-50">${price.smHourlyRate.toFixed(2)}</td>
            {/* Adjustable -$X control */}
            <td className="px-3 py-2">
              <div className="flex items-center justify-center gap-1">
                <button
                  onClick={() => onAdjusterChange(Math.max(1, adjuster - 1))}
                ...
                <span className="text-xs font-bold text-red-500 min-w-[36px] text-center">-${adjuster}</span>
                <button
                  onClick={() => onAdjusterChange(Math.min(50, adjuster + 1))}
      ...
      <div className="bg-green-50 border-t border-green-100 px-4 py-3 flex items-center justify-between">
        <div>
          <p className="text-xs text-green-700 font-semibold">% Savings</p>
          <p className="text-sm font-bold text-green-800">{price.savingsPercent}% <span className="text-xs font-normal text-green-600">Per Month</span></p>
        </div>
        <div className="text-right">
          <p className="text-xs text-green-700 font-semibold">SmartMathz Investment</p>
          <p className="text-sm font-bold text-green-800">${price.smInvestment} <span className="text-xs font-normal text-green-600">Per Month</span></p>
        </div>
      </div>
    </div>
  );
...
export default function ResultsPage() {
  ...
  // Adjuster — starts at SM_ADJUSTER ($20), admin can raise or lower it
  const [adjuster, setAdjuster] = useState<number>(SM_ADJUSTER);

  // Custom package standard rate — monthly hours is auto from subjects × 4
  const [customStdRate, setCustomStdRate] = useState<number>(50);
  ...
  // ── Auto-calculate custom monthly hours from subjects × 4 ─────────────────
  const customWeeklyHours  = customSubjects.reduce((s, c) => s + c.hours, 0);
  const customMonthlyHours = customWeeklyHours * 4;  // auto: weeks × 4
  ...
  const computedPrice: ComputedPrice | null = useMemo(() => {
    if (selectedPackage.id === 'custom') {
      ...
      return computePriceWithAdjuster(customStdRate, customMonthlyHours, adjuster);
    }
    if (!pricingTable) return null;
    const pp = selectedPackage.id === 'I'   ? pricingTable.packageI
             : selectedPackage.id === 'II'  ? pricingTable.packageII
             : pricingTable.packageIII;
    return computePriceWithAdjuster(pp.standardHourlyRate, pp.monthlyHours, adjuster);
  }, [selectedPackage, pricingTable, customStdRate, customMonthlyHours, adjuster]);
  ...
  const openModal = (s: StudentResult) => {
    ...
    setInstructorName('');
    setCustomStdRate(50);
    setAdjuster(SM_ADJUSTER);  // reset to $20 on each new student
  };
              ...
              <div className="px-6 py-5 space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {PACKAGES.map(pkg => (
                      <button key={pkg.id}
                        onClick={() => { setSelectedPackage(pkg); if (pkg.id !== 'custom') setCustomSubjects([]); setAdjuster(SM_ADJUSTER); }}
                        className={`p-3 rounded-xl border text-left cursor-pointer transition-all
                          ${selectedPackage.id === pkg.id
                            ? 'bg-[#1a2e05] text-white border-[#1a2e05] shadow-sm'
                            : 'bg-white text-gray-700 border-gray-200 hover:border-green-400'}`}>
                ...
                {selectedPackage.id !== 'custom' && pricingTable && computedPrice && (
                  <>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center text-xs">
                        {[
                          ...
                          const smFee  = Math.round(smRate * pp.monthlyHours);
                          return (
                            <div key={label} className={`rounded-xl p-2 border ${isActive ? 'border-green-500 bg-green-50' : 'border-gray-100 bg-gray-50'}`}>
                              <p className={`font-bold mb-1 ${isActive ? 'text-green-700' : 'text-gray-600'}`}>{label}</p>
                              <p className="text-gray-500">Std: <span className="font-semibold text-gray-700">${pp.standardHourlyRate}/hr</span></p>
                              <p className="text-green-600">SM: <span className="font-semibold">${smRate}/hr</span></p>
                              <p className="text-gray-400 mt-1">${pp.standardMonthlyFee}/mo → <span className="text-green-600 font-semibold">${smFee}/mo</span></p>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    {/* Full breakdown with adjustable adjuster */}
                    <PriceCard
                      price={computedPrice}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input type="number" min={30} max={200} step={0.5} value={customStdRate}
                            onChange={e => setCustomStdRate(parseFloat(e.target.value))}
                            className="w-full px-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500/20 bg-white" />
                          <p className="text-xs text-green-600 mt-1">SM rate: ${(customStdRate - adjuster).toFixed(2)}/hr</p>
                        </div>
                        <div>
                          {/* Auto-calculated from subjects — read only display */}
