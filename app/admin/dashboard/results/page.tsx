...
// // FIND and REPLACE the entire computePriceWithAdjuster function:

// function computePriceWithAdjuster(
//   standardHourlyRate: number,
//   monthlyHours: number,      // package default — used for standard rate (never changes)
//   adjuster: number,
//   customSessions?: number,   // admin-adjusted sessions — used for SM offer only
// ): ComputedPrice {
//   const smSessions = customSessions ?? monthlyHours;
...
//   // SM offer uses the adjusted session count + discounted hourly rate
//   const smHourlyRate   = standardHourlyRate - adjuster;
//   const smMonthlyFee   = Math.round(smHourlyRate * smSessions);
...
//   return {
//     sessions: smSessions,          // the adjusted session count shown in SM column
//     standardHourlyRate,
//     standardMonthlyFee,            // always based on original monthlyHours
//     smHourlyRate,
//     smMonthlyFee,                  // based on adjusted sessions
//     smBiweekly,
//     savingsPercent,
//     smInvestment,
//   };
...
// // ── PriceCard — shows pricing table with adjustable sessions + adjuster ────────
// function PriceCard({
//   price, packageLabel, adjuster, onAdjusterChange,
//   sessions, onSessionsChange, defaultSessions,
// }: {
//   price: ComputedPrice;
//   packageLabel: string;
//   adjuster: number;
...
//   sessions: number;
//   onSessionsChange: (val: number) => void;
...
//             <td className="px-3 py-2 text-center text-sm font-bold text-yellow-600 bg-yellow-50">{sessions}</td>
...
//   const [sessions, setSessions]   = useState<number>(8);           // custom session count
...
//       return computePriceWithAdjuster(customStdRate, customMonthlyHours, adjuster, sessions);
//     }
...
//     return computePriceWithAdjuster(pp.standardHourlyRate, pp.monthlyHours, adjuster, sessions);
//   }, [selectedPackage, pricingTable, customStdRate, customMonthlyHours, adjuster, sessions]);
...
//     setSessions(
//       suggested.id === 'I'   ? 8  :
//       suggested.id === 'II'  ? 16 : 20
//     );
//   };

//   // When package changes, reset sessions to that package's default
...
//       pkg.id === 'II'     ? 16 :
//       pkg.id === 'III'    ? 20 :
//       pkg.id === 'custom' ? customMonthlyHours : 8
//     );
//   };
...
//                           const smFee  = Math.round(smRate * sessions);
//                           return (
...
//                     {/* Custom PriceCard with both adjusters */}
//                     {computedPrice && (
//                       <PriceCard
//                         price={computedPrice}
...
// // ── Price computation ─────────────────────────────────────────────────────────
// // Normal mode:  SM rate = standard rate − adjuster; SM fee = SM rate × sessions.
// // Budget mode:  parentBudget > 0 → SM fee = budget; SM rate = budget ÷ sessions.
...
// // ── PriceCard — pricing table with sessions, rate adjuster + parent budget ────
// function PriceCard({
//   price, packageLabel, adjuster, onAdjusterChange,
//   sessions, onSessionsChange, defaultSessions,
...
//   // Pricing adjusters — everything starts at zero / package default
//   const [adjuster, setAdjuster]         = useState<number>(0); // $ off per hour, default $0
//   const [sessions, setSessions]         = useState<number>(8); // reset to package default on open
...
//     if (selectedPackage.id === 'custom') {
//       if (customMonthlyHours === 0) return null;
//       return computePriceWithAdjuster(customStdRate, customMonthlyHours, adjuster, sessions, parentBudget);
//     }
//     if (!pricingTable) return null;
//     const pp = selectedPackage.id === 'I'  ? pricingTable.packageI
//              : selectedPackage.id === 'II' ? pricingTable.packageII
//              : pricingTable.packageIII;
//     return computePriceWithAdjuster(pp.standardHourlyRate, pp.monthlyHours, adjuster, sessions, parentBudget);
//   }, [selectedPackage, pricingTable, customStdRate, customMonthlyHours, adjuster, sessions, parentBudget]);

//   // ── Open modal — all adjusters reset to zero / package default ─────────────
//   const openModal = (s: StudentResult) => {
//     setSelectedStudent(s);
//     const suggested = getSuggestedPackage({ math: s.math_score, ela: s.ela_score, science: s.science_score });
//     setSelectedPackage(suggested);
...
//     setParentBudget(0);
//     setInstructorComment('');
//     // Sessions default to the suggested package's own default (no adjustment)
//     setSessions(
//       suggested.id === 'I'   ? 8  :
//       suggested.id === 'II'  ? 16 : 20
//     );
...
//   // When package changes, reset all adjusters to that package's defaults
//   const handlePackageChange = (pkg: PackageOption) => {
//     setSelectedPackage(pkg);
//     if (pkg.id !== 'custom') setCustomSubjects([]);
//     setAdjuster(0);
