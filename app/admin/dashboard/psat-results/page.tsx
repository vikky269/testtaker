...
// app/admin/dashboard/psat-results/page.tsx
// Admin review of all PSAT diagnostic submissions — mirrors the GED admin
// review page exactly (view Q&A across the 4 modules, download report,
// delete). Never imports the question pools — only reads what each
// submission already stored.
...
export default function PsatResultsPage() {
  const [subs, setSubs] = useState<PsatSubmission[]>([]);
  const [filtered, setFiltered] = useState<PsatSubmission[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [viewTarget, setViewTarget] = useState<PsatSubmission | null>(null);
  const [menu, setMenu] = useState<{ sub: PsatSubmission; x: number; y: number } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<PsatSubmission | null>(null);
  ...
  const openMenu = (e: React.MouseEvent, sub: PsatSubmission) => {
    e.stopPropagation();
  ...
  const handleDownload = (sub: PsatSubmission) => {
    try {
      generatePsatReport({
  ...
  const handleDelete = async () => {
    ...
    toast.success(`${deleteTarget.full_name}'s PSAT result deleted.`);
    setSubs(prev => prev.filter(s => s.id !== deleteTarget.id));
    setDeleteTarget(null);
  };

  if (loading) return <div className="p-8 text-gray-500">Loading PSAT submissions...</div>;

  return (
    <div className="max-w-7xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">PSAT Assessments</h1>
        <p className="text-sm text-gray-500 mt-1">
          Every SmartMathz PSAT Readiness Diagnostic submission. View full answers or download the report.
        </p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        <div className="bg-white rounded-2xl p-4 text-center border border-gray-100">
          <p className="text-2xl font-extrabold text-gray-800">{subs.length}</p>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {['#', 'STUDENT', 'OVERALL', 'READING & WRITING', 'MATH', 'DATE', 'ACTIONS'].map(h => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-12 text-gray-400 text-sm">No PSAT submissions yet.</td></tr>
              ) : filtered.map((s, idx) => (
      ...
      {/* ── View Details modal — full Q&A, module by module ── */}
      {viewTarget && (
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-3xl z-10">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-0.5">
                  PSAT Diagnostic · {fmtDate(viewTarget.created_at)} · Overall {viewTarget.overall_score != null ? `${Math.round(viewTarget.overall_score)}%` : '—'}
                </p>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-1.5">Delete this PSAT result?</h3>
            <p className="text-sm text-gray-500 mb-1">
              You're about to permanently delete
              <span className="font-semibold text-gray-800"> {deleteTarget.full_name}</span>'s PSAT submission.
            </p>
